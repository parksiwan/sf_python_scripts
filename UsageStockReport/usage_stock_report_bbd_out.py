...
def main():
    # Files to be read
    sf_current_stock = 'sf_current_stock_bbd_out.csv'
    ...
    for row_code in df_sf_code_list.itertuples():
        ...
        if len(df_stock_of_code) > 0:
            current_date = datetime.datetime.strptime(df_stock_of_code.iat[0, 1], "%Y-%m-%d")  #"%d/%m/%Y")  NEED TO CHANGE ON WINDOWS
            #print(current_date)
            temp_data_list = []
            ...
            for row_stock_of_code in df_stock_of_code.itertuples():
                ...
                temp_bbd = datetime.datetime.strptime(row_stock_of_code[6], "%Y-%m-%d")  #"%d/%m/%Y")  NEED TO CHANGE ON WINDOWS
                if temp_bbd > current_date:
            ...
            for i in range(len(temp_stock_of_code)):
                if temp_stock_of_code.iat[i, 1] > current_date:  # calculate only when bbd > current_date(finishing date) 
            ...
            stock_lasting = 'Dead Stock' if ave_usage == 0 else ''
            ...
            data_list[first_line_index - 1]['stock_waste'] = total_current_stock   #temp_stock_of_code.iat[0, 5]
    ...
    writer = pd.ExcelWriter('sf_usage_stock_report_bbd_out.xlsx', engine='xlsxwriter')
